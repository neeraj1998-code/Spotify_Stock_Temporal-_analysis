...
```{r}
spotify.data$d1_rollavg_Volume <- scale(roll_mean(spotify.data$Volume, n = 1, align = "right", fill = NA))
```


```{r}
spotify.data$d1_rollavg <- scale(roll_mean(spotify.data$Open, n = 1, align = "right", fill = NA))
spotify.data$d2_rollavg <- scale(roll_mean(spotify.data$Open, n = 2, align = "right", fill = NA))
```


```{r}
spotify.data$d3_rollavg <- scale(roll_mean(spotify.data$Open, n = 3, align = "right", fill = NA))
spotify.data$d4_rollavg <-scale( roll_mean(spotify.data$Open, n = 4, align = "right", fill = NA))
spotify.data$d5_rollavg <-scale( roll_mean(spotify.data$Open, n = 5, align = "right", fill = NA))
spotify.data$d6_rollavg <-scale( roll_mean(spotify.data$Open, n = 6, align = "right", fill = NA))
spotify.data$d7_rollavg <- scale(roll_mean(spotify.data$Open, n = 7, align = "right", fill = NA))
```
...
### ? Time Series Formulation - Why freq of 7
```{r}
data.ts <- ts(spotify.data$Open, freq = 7)
#data.ts
```
...
### ETS Smoothing
Training & Test Set ? Why new ts object every line, why not use the one formulated before
```{r}
...
model.lr <- glm(Direction ~ d1_rollavg+d2_rollavg+d3_rollavg+d4_rollavg+d5_rollavg+d6_rollavg+d7_rollavg, data = train_new, family = "binomial",maxit=50)
...
```{r}
glm.probs <- predict(model.lr, type='response', newdata=test_new)
...
```{r}
library(dplyr)
train_new=ts(train_new)
test_new=ts(test_new)
lm.model=tslm(Open~d1_rollavg+d2_rollavg+d3_rollavg+d4_rollavg+d5_rollavg+d6_rollavg+d7_rollavg,train_new)
```
...
```{r}
forecasts_lm=predict(lm.model,h=99, newdata = test_new)
```
